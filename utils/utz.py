...
class TimeZoneDatabase(object):
    ...
    def _pack_rules(self, rule_groups: Dict[str, List[Rule]], c_buf: List[str], h_buf: List[str]):
        ...
        c_buf.append('};')
        h_buf.append(f'extern const urule_packed_t zone_rules[{idx}];')
    ...
    def _pack_zones(self, rule_groups, rule_group_starts, c_buf, h_buf):
        ...
        h_buf.extend(
            [f'extern const char zone_abrevs[{total_char}];', ''])
        ...
        c_buf.append('};')
        h_buf.append(
            f'extern const uzone_packed_t zone_defns[{len(packed_zones)}];')
    ...
    def _pack_links(self, zone_indexes, c_buf, h_buf, included_aliases=[]):
        ...
        h_buf.append(f'extern const unsigned char zone_names[{total_char}];')
